			...
			<body>
				<div class="container-md" style="position:relative">
					...
					>
						...
						<footer
							...
							style="display:${buildTime
								...
								: "none"};font-size:20px"
						>
							<cite>Build Time : ${buildTime} ( IP : ${ip} ) </cite>
						</footer>
						...
						${extraInfo && typeof extraInfo ==='object' && Object.keys(extraInfo).map(
							v => html`
								<br />
								<footer
									class="blockquote-footer"
									style="display:${v && extraInfo[v]
										? "inline-block"
										: "none"};font-size:20px"
								>
									<cite>${v} : ${extraInfo[v]} </cite>
								</footer>
							`,
						)}
					</blockquote>
								...
								(v, p) => html`
									<tr>
										...
										<td class="text-success">
											...
											<span
												class="text-danger"
												style="font-size:12px;display:${v.latestVersion !==
												v.version
													? "inline-block"
													: "none"}"
											>
												&nbsp;(&nbsp;<cite>${v.latestVersion}</cite>&nbsp;)&nbsp;
											</span>
										</td>
										<td class="text-success">
											<span>
												...
												<span
													class="text-danger"
													style="font-size:12px;display:${v.latestVersion !==
														v.version && v.latestTime
														? "inline-block"
														: "none"}"
												>
													&nbsp;(&nbsp;<cite>${v.latestTime}</cite>&nbsp;)&nbsp;
												</span>
											</span>
										</td>
										<td class="text-dark">${v.belong}</td>
									</tr>
								`,
							)}
						</tbody>
					...
					<footer class="text-center">
						...
						<a
							target="_blank"
							href="https://www.npmjs.com/package/package-version-plugin"
						>
							<cite>package-version-plugin@${pkgVersion}</cite>
						</a>
					</footer>
				</div>
